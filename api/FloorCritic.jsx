...
// ─── Upload a file directly to Gemini Files API via resumable upload ───
// Browser → Gemini, bypassing Vercel’s 4.5MB body limit
...
if (!urlRes.ok) {
let errorDetail = `${urlRes.status}`;
try {
const raw = await urlRes.text();
try {
const errJson = JSON.parse(raw);
if (errJson.error) errorDetail = errJson.error;
else errorDetail = raw.slice(0, 150);
} catch {
errorDetail = raw.slice(0, 150);
}
} catch { /* body unreadable */ }
if (urlRes.status === 404) {
throw new Error(“The /api/upload-url endpoint is missing. Make sure api/upload-url.js was deployed to Vercel.”);
}
throw new Error(`Upload session failed (${urlRes.status}): ${errorDetail}`);
}
const urlRaw = await urlRes.text();
let urlData;
try { urlData = JSON.parse(urlRaw); }
catch { throw new Error(`upload-url returned non-JSON: ${urlRaw.slice(0, 200)}`); }
const { uploadUrl } = urlData;
if (!uploadUrl) {
throw new Error(“upload-url endpoint returned no URL”);
}

```
// Step 2: PUT bytes directly to Gemini using XHR (so we get progress events)
const geminiResponse = await new Promise((resolve, reject) => {
  ...
  xhr.onload = () => {
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      catch { reject(new Error("Invalid Gemini response: " + xhr.responseText.slice(0, 100))); }
    } else {
...
if (!geminiResponse.file?.name) {
  throw new Error("Gemini did not return a file handle");
}
return geminiResponse.file; // { name, uri, mimeType, state, ... }
```
...
```
try {
  // Step 1: Transcode HEVC → H.264 if needed
  ...
  // Step 2: Generate thumbnails for UI
  ...
  // Step 3: Upload each video directly to Gemini Files API
  const uploadedFiles = []; // { name, mimeType }
  for (let i = 0; i < preparedVideos.length; i++) {
    const pv = preparedVideos[i];
    const sizeMB = (pv.file.size / 1024 / 1024).toFixed(1);
    setProgress(`Uploading video ${i + 1}/${preparedVideos.length} (${sizeMB} MB) to Gemini…`);
    const file = await uploadFileToGemini(pv.file, pv.mimeType, (ratio) => {
      const pct = Math.round(ratio * 100);
      setProgress(`Uploading video ${i + 1}/${preparedVideos.length} — ${pct}% (${sizeMB} MB)`);
    });
    uploadedFiles.push({ name: file.name, mimeType: pv.mimeType });
  }
...
```
  // Step 5: Trigger analysis — Gemini processes the files (can take 30-120s for videos)
  setProgress("Analysing with Gemini 2.5 Pro… (30–120s)");

  const response = await fetch("/api/analyse", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ systemPrompt, userPrompt, files: uploadedFiles })
  });
  ...
  if (!response.ok) {
    ...
    try {
      ...
      try {
        ...
        else errMsg = `API ${response.status}: ${raw.slice(0, 200)}`;
      } catch {
        // Not JSON — use raw text (first chars)
        errMsg = `API ${response.status}: ${raw.slice(0, 200)}`;
      }
    } catch {
      errMsg = `API ${response.status} (no response body)`;
    }
  ...
  let parsedResponse;
  try { parsedResponse = JSON.parse(raw); }
  catch { throw new Error(`Server did not return JSON: ${raw.slice(0, 200)}`); }
  const { text } = parsedResponse;
  if (!text) throw new Error("Analysis response had no text field");
...
return (
<div style={bg}>
<style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;1,400&family=Lora:ital@0;1&family=DM+Mono:wght@300;400;500&display=swap'); * { box-sizing: border-box; } ::-webkit-scrollbar { width: 4px; }  ::-webkit-scrollbar-track { background: transparent; } ::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.1); border-radius: 2px; } input[type=range] { -webkit-appearance: none; width: 100%; height: 4px; background: rgba(255,255,255,0.12); border-radius: 2px; outline: none; } input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; width: 16px; height: 16px; border-radius: 50%; background: #E8C547; cursor: pointer; } @keyframes fadeUp { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } } @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } } @keyframes spin { to { transform: rotate(360deg); } } @keyframes pulse { 0%,100% { opacity: 0.5; } 50% { opacity: 1; } }`}</style>
      ...
      <div style={{ animation: "fadeUp 0.5s ease forwards", paddingTop: 36 }}>
        ...
        <section style={{ marginBottom: 36 }}>
          ...
          <div style={{ marginTop: 12, padding: "10px 12px", background: "rgba(71,181,232,0.06)", border: "1px solid rgba(71,181,232,0.15)", borderRadius: 8, fontFamily: "'DM Mono', monospace", fontSize: 10, color: "rgba(240,236,224,0.55)", lineHeight: 1.6 }}>
            💡 <strong style={{ color: "#47B5E8" }}>Full video analysis:</strong> FloorCritic analyses the complete video with audio to evaluate musicality and timing. Upload up to 3 angles for best results. Max 500MB per video.
          </div>
        </section>
